...
<body>
  <?php
    include "../conn.php";
              ...
              $tgl_awal_raker = ("2023-02-06");
              $tgl_akhir_raker = ("2023-02-10");
      ...
      <?php
          ...
          if(($tgl_today >= $tgl_awal_raker) && ($tgl_today <= $tgl_akhir_raker)){
            if(isset($_POST['kode_qr'])){
              ...
              // IF == 1
              $sql1 = mysqli_query($koneksi, "SELECT * FROM peserta WHERE nip='$_POST[kode_qr]'") Or die (mysqli_error($koneksi));
              if(mysqli_num_rows($sql1) == 1){

                // IF == 0
                $sql0 = mysqli_query($koneksi, "SELECT * FROM kehadiran WHERE nik='$_POST[kode_qr]' AND event='$event' AND tanggal='$tgl_today' ") Or die (mysqli_error($koneksi));
                if(mysqli_num_rows($sql0) == 0){
                    ...
                    if($insert){
                      echo '<script>sweetAlert({
                                                                title: "SUCCESS", 
                                                                    text: "Terimakasih sudah hadir di Event Rakernas \n PT PESONNA OPTIMA JASA Tahun 2023", 
                                                                    type: "success",
                                        timer : 800
                                                                    });</script>';
                            }
              ...
              // .IF == 1
                else
                {
                  echo '<script>sweetAlert({
                                                                title: "WARNING", 
                                                                text: "Mohon maaf Anda tidak terdaftar sebagai peserta Rakernas \n PT PESONNA OPTIMA JASA Tahun 2023", 
                                                                type: "warning",
                                                                timer : 2000
                                                                });</script>';
                }
            ...
            else
            {
              echo '<script>sweetAlert({
                    title: "FAILED", 
                    text: "Mohon Maaf waktu Absensi Kehadiran belum dibuka", 
                    type: "error",
                    timer : 2000
                    });</script>';
            }
...
<?php
  //  $query1="select kehadiran.*, karyawan.* from kehadiran, karyawan where kehadiran.nik=karyawan.nik AND kehadiran.status='Hadir' AND kehadiran.event='$event' AND kehadiran.tanggal BETWEEN '$tgl_awal_raker' AND '$tgl_akhir_raker' ORDER BY kehadiran.nik ASC";
   $query1="select kehadiran.*, peserta.* from kehadiran, peserta where kehadiran.nik=peserta.nip AND kehadiran.status='Hadir' AND kehadiran.event='$event' AND kehadiran.tanggal BETWEEN '$tgl_awal_raker' AND '$tgl_akhir_raker' ORDER BY kehadiran.tanggal DESC, kehadiran.waktu DESC";
